...
{p 8 15 2}
{cmd:dependencies}, {it:{opt subcommand}} [{it: = freeze | unfreeze | which | remove }]  [ {opt using(filename)} {opt adolist(string)} {opt all} {opt replace} ]
{p_end}
...
{synopt :{opt freeze}}save a {it:zipfile} with the dependencies passed in {it:adolist} or {it:all}{p_end}
{synopt :{opt unfreeze}}unzip to {it:dependencies} adopath the frozen dependencies {it:zipfile}{p_end}
...
{synoptline}
{syntab :Main: {it:required for freeze/unfreeze}}
{synopt :{opt using}({it:filename}{cmd:)}}indicates the {it:zipfile} to be written/read{p_end}
{synoptline}
...
{synopt :{opt all}}freeze everything in the {it:plus} adopath (cannot be combined with {it:adolist}){p_end}
{synopt :{opt replace}}freeze will replace the {it:using(filename)} if it already exists{p_end}
{synoptline}
...
{phang} {cmdab:freeze} save a zip file with the current version of the dependencies. Requires option {opt using}({it:filename}{cmd:)} and either {opt adolist}({it:string}{cmd:)} or {opt all}.

{phang} {cmdab:unfreeze} unzip to {it:dependencies} adopath the frozen dependencies from a zipfile. Requires option {opt using}({it:filename}{cmd:)}.
...
{phang} {cmdab:using(}{it:filename}{cmd:)} indicates the zip file to be written if subcommand {opt freeze} or read if subcommand {opt unfreeze}.

{dlgtab:Others}

{phang} {cmdab:adolist(}{it:string}{cmd:)} freeze a selection of commands, passed in {it:adolist}. Cannot be combined with {it:all}. Multiple commands and packages can be passed in the same {it:string}, separated by spaces. First, the {it:plus} adopath registry of installed packages -{it:stata.trk}- is searched for matching packages, then, remaining commands are searched as standalone files in the adopath by {it:{help "findfile"}}. Commands that are not found will trigger a warning display but won't cause errors.

{phang} {cmdab:all} freeze everything in the {it:plus} adopath for which a corresponding entry in the {it:plus} adopath registry of installed packages -{it:stata.trk}- exists. Cannot be combined with {it:adolist}.

{phang} {cmdab:replace} freeze will replace the zip file specified in {it:using(filename)} if it already exists.
...
{phang2}. {stata `"dependencies, freeze adolist(iegraph iematch) using(dependencies_myproject.zip)"'}{p_end}

{pstd}Note that you could also have listed the whole package instead of naming just some of its components:{p_end}
{phang2}. {stata `"dependencies, freeze adolist(ietoolkit) using(dependencies_myproject.zip) replace"'}{p_end}
...
{phang2}. {stata `"dependencies, freeze adolist(ietoolkit iegraph iematch) using(dependencies_myproject.zip) replace"'}{p_end}

{pstd}For reproducibility, your master run do file will NOT install {cmd:ietoolkit} from SSC. Instead, you unfreeze the dependencies at the beginning:{p_end}
